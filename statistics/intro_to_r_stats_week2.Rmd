...
sfo_feb_flights %>%
  summarise(mean = mean(arr_delay), stdev = sd(arr_delay), n = n(), min = min(arr_delay), max = max(arr_delay))
```
...
data <- sfo_feb_flights %>%
  group_by(carrier) %>%
  summarise(median_ad = median(arr_delay), iqr_ad = IQR(arr_delay), stddev_ad = sd(arr_delay))


data
#visualize the summary
ggplot(data = sfo_feb_flights , aes(x = carrier, y = arr_delay)) + geom_boxplot()
...
#LGA, shown in previous code block
```
...
nycflights %>%
  ggplot(aes(x = avg_speed, y = distance)) + geom_line()
```
...
nycflights %>%
  group_by(dep_type) %>%
  summarise(ot_arr_rate = sum(arr_type == "on time") / n(), dl_arr_rate = sum(arr_type == "delayed")/n())  


ggplot(data = nycflights, aes(x = dep_type, fill = arr_type)) +
  geom_bar()
```
